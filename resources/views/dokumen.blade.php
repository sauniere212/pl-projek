...
<head>
    ...
    <title>Dokumen - DISPERUMKIM Kota Bogor</title>
</head>
<body>
    <!-- Navbar -->
    ...
    <!-- Dokumen Section -->
    <div style="background: #f2f2f2; padding: 40px 0 60px 0; min-height: 350px;">
        <div style="max-width: 1200px; margin: 0 auto;">
            <div style="text-align: center; font-size: 20px; margin-bottom: 30px; font-family: Arial, sans-serif; color: #333;">
                Dokumen <span style="color: #e48b3c; font-weight: bold;">DISPERUMKIM Kota Bogor</span>
            </div>
            <div style="background: #fff; border-radius: 6px; box-shadow: 0 2px 8px rgba(0,0,0,0.04); padding: 24px;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
                    <div>
                        Tampilkan
                        <select style="padding: 4px 8px; border-radius: 4px; border: 1px solid #ccc;" id="itemsPerPage">
                            <option>10</option>
                            <option>25</option>
                            <option>50</option>
                            <option>100</option>
                    ...
                    <div>
                        <input type="text" placeholder="Cari" style="padding: 4px 10px; border-radius: 4px; border: 1px solid #ccc; width: 220px;" id="searchInput">
                    </div>
                </div>
                    ...
                    <table style="width: 100%; border-collapse: collapse; font-size: 15px;">
                        <thead>
                            <tr style="background: #f7f7f7; color: #333;">
                                <th class="text-center" style="border: 1px solid #e0e0e0; padding: 8px;">No.</th>
                                <th class="text-center" style="border: 1px solid #e0e0e0; padding: 8px;">Nama Dokumen</th>
                                <th class="text-center" style="border: 1px solid #e0e0e0; padding: 8px;">Keterangan/Tentang</th>
                                <th class="text-center" style="border: 1px solid #e0e0e0; padding: 8px;">Kategori</th>
                                <th class="text-center" style="border: 1px solid #e0e0e0; padding: 8px;">File</th>
                            </tr>
                        </thead>
                        <tbody>
                            @if($dokumens->count() > 0)
                                @foreach($dokumens as $index => $dokumen)
                                    <tr>
                                        <td style="border: 1px solid #e0e0e0; padding: 8px; text-align: center;">{{ $index + 1 }}</td>
                                        <td style="border: 1px solid #e0e0e0; padding: 8px;">{{ $dokumen->nama }}</td>
                                        <td style="border: 1px solid #e0e0e0; padding: 8px;">{{ $dokumen->keterangan }}</td>
                                        <td style="border: 1px solid #e0e0e0; padding: 8px; text-align: center;">{{ $dokumen->kategori }}</td>
                                        <td style="border: 1px solid #e0e0e0; padding: 8px; text-align: center;">
                                            <button onclick="downloadDokumen('{{ asset('storage/' . $dokumen->file_path) }}', '{{ $dokumen->judul }}')" style="padding: 4px 10px; border-radius: 4px; border: 1px solid #e48b3c; background: #e48b3c; color: #fff; cursor: pointer;">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 18px; font-size: 14px; color: #888;">
                    <div class="pagination-info">
                        Menampilkan halaman 1 dari 1
                    </div>
                    <div class="pagination-buttons">
                        <button class="prev-btn" style="padding: 4px 10px; border-radius: 4px; border: 1px solid #ccc; background: #f7f7f7; color: #aaa; margin-right: 4px;" disabled>Sebelumnya</button>
                        <span class="page-btn">1</span>
                        <button class="next-btn" style="padding: 4px 10px; border-radius: 4px; border: 1px solid #e48b3c; background: #e48b3c; color: #fff; margin-right: 4px;">Berikutnya</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        function downloadDokumen(fileUrl, fileName) {
            // Buat link download
