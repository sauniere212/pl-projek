                        ...
                        <table class="table table-bordered table-striped" id="templateTable">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $template->nama_pejabat }}</td>
                                    <td>{{ $template->jabatan }}</td>
                                    <td>
                                        @if($template->foto_pejabat)
                                            <img src="{{ asset('storage/' . $template->foto_pejabat) }}" 
                                                 alt="Foto Pejabat" 
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Tambah Template Sambutan</h5>
            ...
            <form id="addTemplateForm" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="foto_pejabat" class="form-label">Foto Pejabat</label>
                        <input type="file" class="form-control" id="foto_pejabat" name="foto_pejabat" accept="image/*">
                        <div class="form-text">Format: JPG, PNG, GIF. Maksimal 2MB</div>
                    </div>
                    <div class="mb-3">
                        <label for="isi_content" class="form-label">Isi Sambutan *</label>
...
<div class="modal fade" id="editTemplateModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Edit Template Sambutan</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form id="editTemplateForm" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="edit_nama_pejabat" class="form-label">Nama Pejabat *</label>
                    ...
                    <div class="mb-3">
                        <label for="edit_foto_pejabat" class="form-label">Foto Pejabat</label>
                        <input type="file" class="form-control" id="edit_foto_pejabat" name="foto_pejabat" accept="image/*">
                    ...
                    <div class="mb-3">
                        <label for="edit_isi_content" class="form-label">Isi Sambutan *</label>
...
$(document).ready(function() {
    ...
    $('#addTemplateForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '{{ route("admin.template-sambutan.store") }}',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    $('#editTemplateForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '{{ route("admin.template-sambutan.update", ":id") }}'.replace(':id', id),
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
...
function editTemplate(id) {
    $.ajax({
        url: '{{ route("admin.template-sambutan.edit", ":id") }}'.replace(':id', id),
        type: 'GET',
        success: function(response) {
            $('#edit_id').val(response.id);
            $('#edit_judul_halaman').val(response.judul_halaman);
            $('#edit_tanggal').val(response.tanggal);
            $('#edit_nama_pejabat').val(response.nama_pejabat);
            $('#edit_jabatan').val(response.jabatan);
            $('#edit_isi_content').val(response.isi_content);
            
            // Tampilkan foto saat ini
            if (response.foto_pejabat) {
                $('#current_photo').html('<img src="{{ asset("storage") }}/' + response.foto_pejabat + '" style="width: 100px; height: 100px; object-fit: cover; border-radius: 5px;">');
            } else {
                $('#current_photo').html('<span class="text-muted">Tidak ada foto</span>');
            }
...
function toggleStatus(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '{{ route("admin.template-sambutan.toggle-status", ":id") }}'.replace(':id', id),
                type: 'PATCH',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
...
function deleteTemplate(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '{{ route("admin.template-sambutan.destroy", ":id") }}'.replace(':id', id),
                type: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
